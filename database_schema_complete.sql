...
BEGIN
  ...
  -- Si el email ya existía (pre-insertado como placeholder), actualiza el id real
...
COMMENT ON FUNCTION public.handle_new_user() IS 'Trigger AFTER INSERT en auth.users. Crea o actualiza el perfil en public.users.';
...
-- Necesario para que el trigger (SECURITY DEFINER) pueda insertar
